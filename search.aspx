...
<script runat="server">
  ...
  Sub Page_Load(Sender As Object, E As EventArgs)       
        ...
        End If      
         ...
         lblsortcat.text = "Sort Search Result:"  
         lblheaderrandom.text = "Featured Recipe"  
         lblPagename.text = "Search Recipe"     
  ...
  End If
        ...
        'Search parameters
         if Request("find") <> "" then
             ...
             strSearchSQL += " OR Author LIKE '%" & Replace(Request("find"),"'","''") & "%'"
             strSearchSQL += " OR Category LIKE '%" & Replace(Request("find"),"'","''") & "%'"
         end if
...
strSQL = "SELECT *, (RATING/NO_RATES) AS Rates FROM Recipes" & strSearchSQL & " AND LINK_APPROVED = 1 ORDER BY " & Replace(recipesqlorderby, "'", "''") & "" 
        ...
        'getcatid = Request.QueryString("find")

  'Get the total record count
  strSQL = "SELECT Count(ID) FROM Recipes" & strSearchSQL

        'Call Open database - connect to the database
        DBconnect()

        objCommand.Connection.Open()
        lblrcdcount.text = "Your search for&nbsp;" & "(&nbsp;" & Request("find") & "&nbsp;)&nbsp;return:&nbsp;" & objCommand.ExecuteScalar() & "&nbsp;records"
'lblrcdcount.Text = "There are&nbsp;" & "( " & objCommand.ExecuteScalar() & " )" & "&nbsp;recipes found" 
...
'Display sort category links
 Sub SortCategoryLink()
        ...
        strfind = Request.QueryString("find")
        ...
        Else 
           LinkMostPopular.NavigateUrl = "search.aspx?find=" & strfind & "&sid=" & 2
           LinkMostPopular.Text = "Most Popular"
           LinkMostPopular.ToolTip = "Sort search by Most Popular Recipes"
        End if
        ...
        Else 
           LinkHighestRated.NavigateUrl = "search.aspx?find=" & strfind & "&sid=" & 1
           LinkHighestRated.Text = "Highest Rated"
           LinkHighestRated.ToolTip = "Sort search by Newest Rated Recipes"
        End if
        ...
        Else 
           LinkNewest.NavigateUrl = "search.aspx?find=" & strfind & "&sid=" & 3
           LinkNewest.Text = "Newest"
           LinkNewest.ToolTip = "Sort search by Newest Recipes"
        End if
        ...
        Else 
           LinkName.NavigateUrl = "search.aspx?find=" & strfind & "&sid=" & 4 & "&ob=" & 1
           LinkName.Text = "Name ASC"
           LinkName.ToolTip = "Sort search by Recipe Name ASC"
        End if
...
<span class="bluearrow">�</span>&nbsp;<asp:HyperLink tooltip="Back to recipe homepage" id="HyperLink1" cssClass="dtcat" runat="server" />&nbsp;<span class="bluearrow">�</span>&nbsp;<asp:Label cssClass="content10" runat="server" id="lblPagename" />
</div>
</td>
    <td width="16%" rowspan="5" valign="top">
<!--Begin Random Recipe-->
